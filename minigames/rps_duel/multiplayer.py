"""Multiplayer metadata/hooks for Rock-Paper-Scissors duel."""

from __future__ import annotations

import random
from typing import Iterable, Dict, Any, Optional

MINIGAME_ID = "rps_duel"
MULTIPLAYER_ENABLED = True
CHOICES = ("rock", "paper", "scissors")


def get_minigame_id() -> str:
    return MINIGAME_ID


def build_match_payload(host_state: Optional[Dict[str, Any]], participants: Iterable[str]) -> Dict[str, Any]:
    """Return any extra payload needed to launch the minigame."""
    return {"minigame": MINIGAME_ID, "participants": list(participants or [])}


def resolve_result(result_payload: Dict[str, Any]) -> Dict[str, Any]:
    """Normalize result payload from the client."""
    return {
        "duel_id": result_payload.get("duel_id"),
        "winner": result_payload.get("winner"),
        "loser": result_payload.get("loser"),
        "outcome": result_payload.get("outcome"),
    }


def ai_choice(seed: str, round_no: int, participants: Iterable[str]) -> str:
    """Return an auto-choice for NPC participation."""
    rng = random.Random(f"{seed}-{round_no}-{sorted(list(participants or []))}")
    return rng.choice(CHOICES)
